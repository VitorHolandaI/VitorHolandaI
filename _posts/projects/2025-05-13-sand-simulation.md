...
A falling-sand simulator built entirely in Python using Tkinter, with no external libraries. Move the mouse over the window and sand pours out, falls under gravity, and piles up at the bottom or on top of already settled grains.
...
A small personal project to explore Tkinter's canvas and event loop. The physics side is still a work in progress, but it already looks and feels like sand.
