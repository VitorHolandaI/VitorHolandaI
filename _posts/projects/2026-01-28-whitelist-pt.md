...
Uma ferramenta de controle de acesso à rede feita para laboratórios universitários. A ideia é simples: em vez de tentar bloquear sites indesejados específicos, o que é uma batalha sem fim, a abordagem é invertida. Ela bloqueia tudo por padrão e abre apenas os domínios que o professor aprovou explicitamente.
...
Duas implementações estão incluídas: uma usando `iptables` puro e outra usando `ufw`. Ambas foram desenvolvidas em conjunto com um colega.
