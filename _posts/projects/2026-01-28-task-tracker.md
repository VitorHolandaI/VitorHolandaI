...
A personal command-line task manager written in Nim, called `Doin`. Tasks are stored as plain JSON files under `~/.tasks/`, organized into three folders: active, done, and overdue. There's no database, so the data is always readable.
